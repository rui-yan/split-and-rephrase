...
def main():
    ...
    test_dirs_path_prefix = base_path + '/Split-and-Rephrase/evaluation-directories/wikisplit/validation/'

    # the file containing the ids of the test sentences
    test_sent_ids_path = base_path + '/Split-and-Rephrase/benchmark/wikisplit/complex-sents/validation.id'

    # a directory that will hold single sentence files for the hypotheses
    test_hypothesis_sents_dir = base_path + '/phrasing/models/wikisplit/validation_complex_output_sents/'
    if not os.path.exists(test_hypothesis_sents_dir):
        os.mkdir(test_hypothesis_sents_dir)

    test_target = base_path + '/phrasing/models/wikisplit/validation.complex.unique.output'
