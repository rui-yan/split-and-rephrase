...
def main():
    ...
    # base_path = '/lfs/turing4/0/ruiyan/NLP'
    ...
    test_dirs_path_prefix = base_path + '/Split-and-Rephrase/evaluation-directories/validation/'

    # the file containing the ids of the test sentences
    test_sent_ids_path = base_path + '/Split-and-Rephrase/benchmark/complex-sents/validation.id'

    # a directory that will hold single sentence files for the hypotheses
    test_hypothesis_sents_dir = base_path + '/phrasing/models/{}/validation_complex_output_sents/'.format(
        model_name)

    if not os.path.exists(test_hypothesis_sents_dir):
        os.mkdir(test_hypothesis_sents_dir)

    results_file_path = base_path + '/phrasing/models/{}/overtime_dev_sprp_eval.txt'.format(
        model_name)

    results_file = codecs.open(results_file_path, 'w','utf8')

    for model in models:
        command = 'python {}/OpenNMT-py@d4ab35ay/translate.py \
                    -gpu 1 \
                    -batch_size 1 \
                    -model {}/phrasing/models/{}/{} \
                    -src {}/Split-and-Rephrase/baseline-seq2seq/validation.complex.unique \
                    -output {}/phrasing/models/{}/validation.complex.unique.output \
                    -beam_size 12 \
                    -verbose \
                    -attn_debug \
                    -replace_unk'
        ...
        test_target = base_path + '/phrasing/models/{}/validation.complex.unique.output'.format(model_name)
